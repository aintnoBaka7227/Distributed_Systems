...
public class MemberProfile {
    ...
    private MemberProfile(String name, int minLatencyMs, int maxLatencyMs, double dropRate) {
        this.name = name;
     ...
     * failure: moderate latency, higher drops (can crash with 'crash' command)
     * standard: moderate latency, low drops
     * @param name profile name
     * @return profile
     */
    public static MemberProfile fromName(String name) {
        String n = name == null ? "standard" : name.toLowerCase();
        switch (n) {
            case "reliable":
    ...
    public boolean beforeNetworkAction() {
        ...
        if (rnd.nextDouble() < dropRate) return false;
        int delay = minLatencyMs;
        if (maxLatencyMs > minLatencyMs) {
            delay += rnd.nextInt(maxLatencyMs - minLatencyMs + 1);
        }
        if (delay > 0) {
